...
export default function LaborViewer({ onTotalsChange }) {
  ...
  // locked to a code for this file
  ...
  // ✅ For display: sort labor types by descending cost
  // IMPORTANT: must be ABOVE early returns to avoid hook-order errors
  const sortedTypes = useMemo(() => {
    return Object.entries(totals.byType).sort(
      (a, b) => (b[1]?.cost || 0) - (a[1]?.cost || 0)
    );
  }, [totals.byType]);

  // ✅ Report totals up to Home (send object w/ breakdown)
  useEffect(() => {
