...
// =====================================================
// MLABOR EDITOR (Primary + AlternateMLabor)
// Endpoints assumed:
//   GET/POST   /api/mlabor
//   PUT        /api/mlabor/{MLaborId}
//   GET        /api/mlabor/alternates/{MLaborId}     (alternates for parent)
//   POST       /api/mlabor/alternates               (body includes MLaborId)
//   PUT        /api/mlabor/alternates/{AlternateId}
// =====================================================
export default function BoxView4({ number, onBack }) {
  const starter = [
    { ...blank, isExisting: false, mLaborId: undefined },
    { ...blankAlt },
    { ...blankAlt },
  ];
  ...
  React.useEffect(() => {
    async function loadPrimariesAndAlternates() {
      try {
        // ✅ MLabor primaries
        const res = await fetch(`${API_BASE}/mlabor`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const rows = await res.json(); // expected: array of { MLaborId, Description, ... }

        const formatted = await Promise.all(
          (Array.isArray(rows) ? rows : []).map(async (r) => {
            const mLaborId = r.MLaborId;
            ...
            // ✅ AlternateMLabor rows for this MLaborId
            let alternates = [];
            try {
              const altRes = await fetch(
                `${API_BASE}/mlabor/alternates/${encodeURIComponent(mLaborId)}`
              );
              ...
              console.warn("Failed to load alternates for", mLaborId, err);
            }
        ...
        console.error("Failed to load mlabor:", err);
        setSections([]);
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (const row of sections) {
        ...
        // ======================
        // CASE 1: Existing MLabor
        // ======================
        if (primary.isExisting && primary.mLaborId) {
          const mLaborId = primary.mLaborId;

          // 1) UPDATE primary
          const updRes = await fetch(`${API_BASE}/mlabor/${encodeURIComponent(mLaborId)}`, {
            method: "PUT",
          ...
          if (!updRes.ok) {
            throw new Error(`MLabor UPDATE failed: ${JSON.stringify(updData)}`);
          }
          ...
          for (const alt of row.slice(1)) {
            ...
            if (alt.isExistingAlt && alt.alternateId) {
              const altUpdRes = await fetch(
                `${API_BASE}/mlabor/alternates/${encodeURIComponent(alt.alternateId)}`,
                {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
              ...
              if (!altUpdRes.ok) {
                throw new Error(`AlternateMLabor UPDATE failed: ${JSON.stringify(altUpdData)}`);
              }
              continue;
            }
            ...
            if (!alt.isExistingAlt) {
              const altRes = await fetch(`${API_BASE}/mlabor/alternates`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  MLaborId: mLaborId,
              ...
              if (!altRes.ok) {
                throw new Error(`AlternateMLabor POST failed: ${JSON.stringify(altData)}`);
              }
            }
          }
        ...
        // CASE 2: New MLabor
        // ======================
        const createRes = await fetch(`${API_BASE}/mlabor`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            Description: String(primary.description ?? ""),
        ...
        if (!createRes.ok) throw new Error(`MLabor POST failed: ${JSON.stringify(createData)}`);

        const mLaborId = createData.MLaborId || createData.mlaborId || createData.id;
        if (!mLaborId) throw new Error("MLaborId missing from MLabor POST response");
        ...
        for (const alt of row.slice(1)) {
          ...
          const altData = await altRes.json().catch(() => ({}));
          if (!altRes.ok) throw new Error(`AlternateMLabor POST failed: ${JSON.stringify(altData)}`);
        }
      }

      alert("✅ Submitted changes to MLabor (updated existing + created new items)!");
    } catch (err) {
