...
// =====================================================
// PLABOR EDITOR (Primary + AlternatePLabor)
// Endpoints assumed:
//   GET/POST   /api/plabor
//   PUT        /api/plabor/{PLaborId}
//   GET        /api/plabor/alternates/{PLaborId}     (alternates for parent)
//   POST       /api/plabor/alternates               (body includes PLaborId)
//   PUT        /api/plabor/alternates/{AlternateId}
// =====================================================
export default function BoxView4({ number, onBack }) {
  const starter = [
    { ...blank, isExisting: false, pLaborId: undefined },
    { ...blankAlt },
    { ...blankAlt },
  ];
  ...
  React.useEffect(() => {
    async function loadPrimariesAndAlternates() {
      try {
        // ✅ PLabor primaries
        const res = await fetch(`${API_BASE}/plabor`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const rows = await res.json(); // expected: array of { PLaborId, Description, ... }

        const formatted = await Promise.all(
          (Array.isArray(rows) ? rows : []).map(async (r) => {
            const pLaborId = r.PLaborId;
            ...
            // ✅ AlternatePLabor rows for this PLaborId
            let alternates = [];
            try {
              const altRes = await fetch(
                `${API_BASE}/plabor/alternates/${encodeURIComponent(pLaborId)}`
              );
              ...
              console.warn("Failed to load alternates for", pLaborId, err);
            }
        ...
        console.error("Failed to load plabor:", err);
        setSections([]);
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (const row of sections) {
        ...
        // ======================
        // CASE 1: Existing PLabor
        // ======================
        if (primary.isExisting && primary.pLaborId) {
          const pLaborId = primary.pLaborId;

          // 1) UPDATE primary
          const updRes = await fetch(`${API_BASE}/plabor/${encodeURIComponent(pLaborId)}`, {
            method: "PUT",
          ...
          if (!updRes.ok) {
            throw new Error(`PLabor UPDATE failed: ${JSON.stringify(updData)}`);
          }
          ...
          for (const alt of row.slice(1)) {
            ...
            if (alt.isExistingAlt && alt.alternateId) {
              const altUpdRes = await fetch(
                `${API_BASE}/plabor/alternates/${encodeURIComponent(alt.alternateId)}`,
                {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
              ...
              if (!altUpdRes.ok) {
                throw new Error(`AlternatePLabor UPDATE failed: ${JSON.stringify(altUpdData)}`);
              }
              continue;
            }
            ...
            if (!alt.isExistingAlt) {
              const altRes = await fetch(`${API_BASE}/plabor/alternates`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  PLaborId: pLaborId,
              ...
              if (!altRes.ok) {
                throw new Error(`AlternatePLabor POST failed: ${JSON.stringify(altData)}`);
              }
            }
          }
        ...
        // CASE 2: New PLabor
        // ======================
        const createRes = await fetch(`${API_BASE}/plabor`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            Description: String(primary.description ?? ""),
        ...
        if (!createRes.ok) throw new Error(`PLabor POST failed: ${JSON.stringify(createData)}`);

        const pLaborId = createData.PLaborId || createData.plaborId || createData.id;
        if (!pLaborId) throw new Error("PLaborId missing from PLabor POST response");
        ...
        for (const alt of row.slice(1)) {
          ...
          const altData = await altRes.json().catch(() => ({}));
          if (!altRes.ok) throw new Error(`AlternatePLabor POST failed: ${JSON.stringify(altData)}`);
        }
      }

      alert("✅ Submitted changes to PLabor (updated existing + created new items)!");
    } catch (err) {
