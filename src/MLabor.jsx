...
import BoxView3 from "./BoxView3"; // ✅ editor for MLabor (make sure BoxView3 uses /mlabor endpoints)
...
export default function DbTestViewer() {
  ...
  // ✅ Alternates for MLabor
  async function fetchAlternates(mLaborId) {
    try {
      ...
      console.warn("Alternates fetch failed for", mLaborId, e);
      return [];
    }
  }
  ...
  async function fetchRows() {
    try {
      ...
      // ✅ 2) For each MLabor row, fetch alternates and attach as .Alternates
      const withAlternates = await Promise.all(
        list.map(async (r) => {
          const alts = await fetchAlternates(r.MLaborId);
  ...
  // ✅ DELETE one row by MLaborId (backend: DELETE /api/mlabor/{id})
  const handleDelete = async (mLaborId) => {
    try {
      setDeletingId(mLaborId);

      const url = `${API_BASE}/mlabor/${encodeURIComponent(mLaborId)}`;
      ...
      setRows((prev) => prev.filter((r) => r.MLaborId !== mLaborId));
    } catch (e) {
  ...
  const toggleExpand = (id) =>
    setRows((prev) =>
      prev.map((r) => (r.MLaborId === id ? { ...r, _expand: !r._expand } : r))
    );
  ...
  if (selected === 1000) {
    return (
      <BoxView3
        ...
        onBack={() => {
          setSelected(null);
          fetchRows(); // ✅ refresh MLabor (and alternates) after returning
        }}
      />
    );
  }
  ...
  return (
    <div className="container py-4">
                    ...
                    <tr key={r.MLaborId || `row-${i}`}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => toggleExpand(r.MLaborId)}
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => handleDelete(r.MLaborId)}
                          disabled={deletingId === r.MLaborId}
                          title="Delete this MLabor item"
                        >
                          {deletingId === r.MLaborId ? "Deleting…" : "Clear"}
                        </button>
                      </td>
                    </tr>
      ...
      <h5 className="mt-3">Code Number 8000</h5>
    </div>
  );
}
