...
import BoxView3 from "./BoxView3"; // ✅ editor for PLabor (make sure BoxView3 uses /plabor endpoints)
...
export default function DbTestViewer() {
  ...
  // ✅ Alternates for PLabor
  async function fetchAlternates(pLaborId) {
    try {
      ...
      console.warn("Alternates fetch failed for", pLaborId, e);
      return [];
    }
  }
  ...
  async function fetchRows() {
    try {
      ...
      // ✅ 2) For each PLabor row, fetch alternates and attach as .Alternates
      const withAlternates = await Promise.all(
        list.map(async (r) => {
          const alts = await fetchAlternates(r.PLaborId);
  ...
  // ✅ DELETE one row by PLaborId (backend: DELETE /api/plabor/{id})
  const handleDelete = async (pLaborId) => {
    try {
      setDeletingId(pLaborId);

      const url = `${API_BASE}/plabor/${encodeURIComponent(pLaborId)}`;
      ...
      setRows((prev) => prev.filter((r) => r.PLaborId !== pLaborId));
    } catch (e) {
  ...
  const toggleExpand = (id) =>
    setRows((prev) =>
      prev.map((r) => (r.PLaborId === id ? { ...r, _expand: !r._expand } : r))
    );
  ...
  if (selected === 1000) {
    return (
      <BoxView3
        ...
        onBack={() => {
          setSelected(null);
          fetchRows(); // ✅ refresh PLabor (and alternates) after returning
        }}
      />
    );
  }
  ...
  return (
    <div className="container py-4">
                    ...
                    <tr key={r.PLaborId || `row-${i}`}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => toggleExpand(r.PLaborId)}
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => handleDelete(r.PLaborId)}
                          disabled={deletingId === r.PLaborId}
                          title="Delete this PLabor item"
                        >
                          {deletingId === r.PLaborId ? "Deleting…" : "Clear"}
                        </button>
                      </td>
                    </tr>
      ...
      <h5 className="mt-3">Code Number 9000</h5>
    </div>
  );
}
